...
namespace Farmer
{
    ...
    public class Diablo
    {
        ...
        {
            const double DPI_SCALING = 1.25;

            Syscall.SetForegroundWindow(window_handle);
            Syscall.GetWindowRect(window_handle, out SysCalls.Syscall.Rect rect);
            Syscall.SetCursorPos((int)(x * DPI_SCALING + rect.left), (int)(y * DPI_SCALING + rect.top));
        }
    ...
    public class FarmingScenario
    {
        ...
        public void ReinitializeDiablo()
        {
            diablo = new Diablo();
        }
        ...
        private ManualResetEvent mre = new ManualResetEvent(true);  // used for game pausing
